...
1. **Session 1 — Connector contract research (no code). ✓ Complete (2026-07-14).** Verify claude.ai's "Add custom connector" flow accepts a manually supplied Client ID/Secret (the settled static-client decision); find the exact callback/redirect URI(s) claude.ai and Claude Desktop use; confirm Streamable HTTP transport expectations. If DCR turns out to be mandatory in practice, stop and revisit (see Risks). *Ends with:* findings recorded in this section (see **Session 1 findings** below); commit is docs-only; push and open the draft PR.
...
**Session 1 findings — connector contract (researched 2026-07-14, against Anthropic's connector docs and the MCP spec):**

1. **Static Client ID/Secret confirmed — the settled decision holds; no DCR needed.** claude.ai's "Add custom connector" flow (Settings → Connectors → Add custom connector → **Advanced settings**) accepts a manually entered OAuth Client ID and Client Secret. Anthropic's own connector-authentication docs explicitly endorse this: supplying "your own pre-registered client ID (and secret, if your server requires one) as static client credentials … avoids dynamic client registration entirely." The Client Secret field is optional in the form (public clients exist), but we supply one — a confidential client is strictly stronger and the SDK's `/token` handler authenticates it. The DCR stop-condition did not trigger; the SDK's `/register` endpoint stays disabled.
2. **Redirect URIs — exactly two, shared by every hosted surface.** claude.ai web, the mobile apps, Claude Desktop Connectors, and Cowork all use the same server-side callback: `https://claude.ai/api/mcp/auth_callback`, with `https://claude.com/api/mcp/auth_callback` as a domain variant Anthropic's docs say to also allow. The provider's static-client record registers **both** as the allowed `redirect_uris` — nothing else. (Claude Code as an OAuth client would need port-agnostic loopback `http://localhost:<port>/callback` matching — irrelevant here since Claude Code stays on the Phase 15 stdio path, decision #8.)
3. **Transport — decision #4 (`stateless=True` + `json_response=True`) is spec-sanctioned, not a shortcut.** Claude supports Streamable HTTP and is deprecating the legacy HTTP+SSE transport. Per the MCP transport spec: a server answering a POSTed JSON-RPC request "MUST either return `Content-Type: text/event-stream` … or `Content-Type: application/json`" and the client "MUST support both" → plain-JSON responses are fully conformant. The GET SSE listening stream is optional (server MAY return 405) and `Mcp-Session-Id` sessions are optional (server MAY assign one) → a stateless server is fully conformant too.
4. **PKCE is mandatory on Claude's side:** every authorization request carries `code_challenge_method=S256` regardless of registration mechanism, and the AS metadata must advertise `code_challenge_methods_supported: ["S256"]` — the SDK's `create_auth_routes` metadata handler does this. Discovery flows from `WWW-Authenticate` → `/.well-known/oauth-protected-resource` → the AS metadata, matching the planned `create_protected_resource_routes` wiring.
5. **Operational data points:** claude.ai connects from Anthropic's cloud infrastructure (server-side, not the browser), so the Render URL just needs to be publicly reachable; the claude.ai/Desktop per-request timeout is **300 seconds**, which comfortably covers the Render free-tier cold start + Neon wake (~30s+) — softens that risk from "may time out" to "slow first call."
...
**Risks / gotchas:**
- **DCR contingency — resolved by session 1:** claude.ai's connector form accepts a supplied Client ID/Secret (Advanced settings), and Anthropic's docs endorse static credentials as the way to avoid DCR entirely. The fallback (enabling the SDK's `/register` with a small clients table) stays documented in case a future surface behaves differently, but it is no longer an open risk for this build.
...
- **Render free-tier sleep:** the OAuth handshake and first tool call after idle eat the cold start (~30s+ spin-up plus the known Neon wake). Session 1 found claude.ai/Desktop's per-request timeout is 300 seconds, so the cold start should complete well inside it — expect a slow first call, not a failure; observe before treating slowness as a bug.
